...
class UserProviderServices {
  ...
  Future addUser(Map data) async {
    await userRef.where("id",isEqualTo: data["id"].toString()).getDocuments().then((value) async{
      if(value.documents.length == 0){
        await FirebaseAuth.instance
          ...
          await userRef.document(signedInUser.user.uid).setData(data).then((value) {
          ...
          if (e.toString() ==
            ...
            responseMap["isSuccess"] = false;
            responseMap["message"] = "Account already exists with this email id";
          } else if (e.toString() ==
